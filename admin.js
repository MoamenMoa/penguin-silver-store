...
async function moveProduct(id,direction){const index=products.findIndex(p=>p.id===id);const target=index+direction;if(index<0||target<0||target>=products.length)return;const buttons=[...document.querySelectorAll('.order-btn')];buttons.forEach(b=>b.disabled=true);try{const ordered=[...products].sort(productOrderCompare);const current=ordered.findIndex(p=>p.id===id),next=current+direction;if(next<0||next>=ordered.length)return;[ordered[current],ordered[next]]=[ordered[next],ordered[current]];const batch=writeBatch(db);ordered.forEach((p,i)=>batch.update(doc(db,'products',p.id),{sortOrder:i,updatedAt:serverTimestamp()}));await batch.commit();await loadProducts();}catch(err){console.error(err);alert('تعذر تغيير ترتيب المنتج: '+(err.message||'خطأ غير معروف'));}finally{buttons.forEach(b=>b.disabled=false);}}
function renderList(){const wrap=$('#adminProducts'),q=$('#adminSearch').value.trim().toLowerCase();wrap.replaceChildren();const list=products.filter(p=>`${p.nameAr||''} ${p.nameEn||''} ${p.sku||''}`.toLowerCase().includes(q));if(!list.length){wrap.append(el('div','empty-state','لا توجد منتجات مطابقة.'));return}for(const p of list){const row=el('div','admin-product');const img=document.createElement('img');img.src=safeUrl(productImages(p)[0]||'assets/logo.png');img.alt='';const body=el('div','');body.append(el('h4','',p.nameAr||p.nameEn||'بدون اسم'),el('small','placement-label',categoryName(p.category)));body.append(el('small','placement-label',p.showOnHome===false?'في القسم فقط':'في القسم والرئيسية'));body.append(el('small','',`${Number(p.price||0).toLocaleString()} EGP · ${Number(p.weight||0).toLocaleString()} g · عيار ${p.karat||'925'} · ${productImages(p).length} صور · مخزون ${Number(p.stock||0)} · ${p.active===false?'مخفي':'ظاهر'}`));const actions=el('div','admin-actions');const up=el('button','small-btn order-btn','↑');up.type='button';up.title='تحريك المنتج لأعلى';up.setAttribute('aria-label','تحريك المنتج لأعلى');up.disabled=products.indexOf(p)===0;up.onclick=()=>moveProduct(p.id,-1);const down=el('button','small-btn order-btn','↓');down.type='button';down.title='تحريك المنتج لأسفل';down.setAttribute('aria-label','تحريك المنتج لأسفل');down.disabled=products.indexOf(p)===products.length-1;down.onclick=()=>moveProduct(p.id,1);const edit=el('button','small-btn','تعديل');edit.type='button';edit.onclick=()=>editProduct(p.id);const del=el('button','small-btn danger','حذف');del.type='button';del.onclick=()=>removeProduct(p.id);actions.append(up,down,edit,del);row.append(img,body,actions);wrap.append(row)}}
